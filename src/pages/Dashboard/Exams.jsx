import React, { useState, useEffect } from "react";
import useExamination from "../../Hook/useExamination"; // Adjust path as needed
import useAuth from "../../Hook/useAuth";
import Pagination from "../../components/Pagination";
import Mtitle from "../../components library/Mtitle"; 
import TableControls from "../../components/TableControls"; 
import SkeletonLoader from "../../components/SkeletonLoader"; 
import MtableLoading from "../../components library/MtableLoading"; 



export default function Exams() {
  const {
    examinations,
    pagination,
    loading,
    error,
    fetchExaminationsByBranch,
    createExamination,
    updateExamination,
    removeExamination,
  } = useExamination();

  const { branch } = useAuth();

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [formData, setFormData] = useState({
    examinationName: "",
    startDate: "",
    endDate: "",
    startTime: "",
    endTime: "",
  });

  // --- Pagination & Filter States ---
  const [currentPage, setCurrentPage] = useState(1);
  const [limitPerPage, setLimitPerPage] = useState(10);
  const [searchTerm, setSearchTerm] = useState("");

  // Format time to 12-hour format with AM/PM
  const formatTimeAmPm = (timeString) => {
    if (!timeString) return "";
    const [hours, minutes] = timeString.split(':');
    let hour = parseInt(hours);
    const minute = minutes;
    const ampm = hour >= 12 ? 'PM' : 'AM';
    hour = hour % 12;
    hour = hour ? hour : 12;
    return `${hour}:${minute} ${ampm}`;
  };

  // Fetch examinations by branch when component mounts, or when page/limit changes
  useEffect(() => {
    fetchExaminationsByBranch(undefined, currentPage, limitPerPage);
  }, [fetchExaminationsByBranch, currentPage, limitPerPage]);

  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setFormData((prev) => ({ ...prev, [name]: value }));
  };

  // --- Filter Handlers ---
  const handleLimitChange = (e) => {
    setLimitPerPage(Number(e.target.value));
    setCurrentPage(1); 
  };

  const handleSearchChange = (e) => {
    setSearchTerm(e.target.value);
    setCurrentPage(1); 
  };

  const openAddModal = () => {
    setFormData({ examinationName: "", startDate: "", endDate: "", startTime: "", endTime: "" });
    setEditingId(null);
    setIsModalOpen(true);
  };

  const openEditModal = (exam) => {
    setFormData({
      examinationName: exam.examinationName,
      // Formatting dates to YYYY-MM-DD for the HTML5 date input
      startDate: exam.startDate ? new Date(exam.startDate).toISOString().split('T')[0] : "",
      endDate: exam.endDate ? new Date(exam.endDate).toISOString().split('T')[0] : "",
      startTime: exam.startTime || "",
      endTime: exam.endTime || "",
    });
    setEditingId(exam._id);
    setIsModalOpen(true);
  };

  const closeModal = () => {
    setIsModalOpen(false);
    setFormData({ examinationName: "", startDate: "", endDate: "", startTime: "", endTime: "" });
    setEditingId(null);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const dataToSubmit = { ...formData, branch };
      if (editingId) {
        await updateExamination(editingId, dataToSubmit);
      } else {
        await createExamination(dataToSubmit);
      }
      closeModal();
      fetchExaminationsByBranch(undefined, currentPage, limitPerPage);
    } catch (err) {
      console.error("Failed to save examination", err);
    }
  };

  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this examination?")) {
      try {
        await removeExamination(id);
        if (examinations.length === 1 && currentPage > 1) {
          setCurrentPage((prev) => prev - 1);
        } else {
          fetchExaminationsByBranch(undefined, currentPage, limitPerPage);
        }
      } catch (err) {
        console.error("Failed to delete examination", err);
      }
    }
  };

  // Logic for local filtering based on Search Term (matching your Subject page pattern)
  const displayedExaminations = searchTerm 
    ? examinations?.filter((exam) => 
        exam.examinationName?.toLowerCase().includes(searchTerm.toLowerCase())
      )
    : examinations;

    

  return (
    <div>
      <div className="p-4 md:p-8 max-w-7xl mx-auto">
        {/* Header Section */}
        <Mtitle
          title="Examinations Management"
          middlecontent={
            <span className="text-sm text-base-content/70 hidden md:inline-block">
              Manage and organize examination schedules for your branch
            </span>
          }
          rightcontent={
            <button onClick={openAddModal} className="btn btn-primary shadow-sm">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
              </svg>
              Add New Examination
            </button>
          }
        />

        {/* Error Alert */}
        {error && (
          <div className="alert alert-error shadow-lg mb-6">
            <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span>{error}</span>
          </div>
        )}

        {/* Table Controls */}
        <div className="bg-base-100 p-4 rounded-xl shadow-sm border border-base-200 mb-6 flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="flex-1 w-full">
            <TableControls
              itemsPerPage={limitPerPage}
              onItemsPerPageChange={handleLimitChange}
              searchTerm={searchTerm}
              onSearchChange={handleSearchChange}
            />
          </div>
        </div>

        {/* Main Table Card */}
        <div className="card bg-base-100 shadow-xl border border-base-200">
          <div className="overflow-x-auto rounded-box">
            <table className="table table-zebra w-full">
              <thead className="bg-base-200 text-base-content text-sm">
                <tr>
                  <th className="py-4 rounded-tl-box">Examination Name</th>
                  <th className="py-4">Start Date</th>
                  <th className="py-4">End Date</th>
                  <th className="py-4">Start Time</th>
                  <th className="py-4">End Time</th>
                  <th className="py-4">Status</th>
                  <th className="py-4 text-right rounded-tr-box pr-8">Actions</th>
                </tr>
              </thead>

              <tbody>
                {loading ? (
                  <SkeletonLoader />
                ) : displayedExaminations?.length === 0 ? (
                  <tr>
                    <td colSpan="7" className="py-12 text-center">
                      <MtableLoading data={displayedExaminations} />
                      <div className="flex flex-col items-center justify-center text-base-content/50 mt-[-40px]">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        <p className="text-lg font-medium">No examinations found</p>
                        <p className="text-sm">Click "Add New Examination" to create one.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  displayedExaminations?.map((exam) => (
                    <tr key={exam._id} className="hover">
                      <td className="py-4">
                        <div className="flex items-center space-x-3">
                          <div className="avatar placeholder">
                            <div className="bg-secondary text-secondary-content rounded-full w-10">
                              <span className="font-bold">{exam.examinationName?.charAt(0).toUpperCase()}</span>
                            </div>
                          </div>
                          <div className="font-semibold text-base">{exam.examinationName}</div>
                        </div>
                      </td>
                      <td className="py-4 text-base-content/80 font-medium">
                        {new Date(exam.startDate).toLocaleDateString()}
                      </td>
                      <td className="py-4 text-base-content/80 font-medium">
                        {new Date(exam.endDate).toLocaleDateString()}
                      </td>
                      <td className="py-4 text-base-content/80 font-medium">
                        {formatTimeAmPm(exam.startTime)}
                      </td>
                      
                      <td className="py-4 text-base-content/80 font-medium">
                        {formatTimeAmPm(exam.endTime)}
                      </td>
                      <td className="py-4">
                        {new Date() > new Date(exam.endDate) ? (
                          <div className="badge badge-ghost badge-sm">Completed</div>
                        ) : new Date() >= new Date(exam.startDate) ? (
                          <div className="badge badge-success badge-sm">Ongoing</div>
                        ) : (
                          <div className="badge badge-info badge-sm">Upcoming</div>
                        )}
                      </td>
                      <td className="py-4 text-right pr-6">
                        <div className="join justify-end">
                          <button
                            onClick={() => openEditModal(exam)}
                            className="btn btn-sm btn-ghost text-info join-item"
                            title="Edit"
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                            </svg>
                            <span className="hidden sm:inline ml-1">Edit</span>
                          </button>
                          <button
                            onClick={() => handleDelete(exam._id)}
                            className="btn btn-sm btn-ghost text-error join-item"
                            title="Delete"
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                            </svg>
                            <span className="hidden sm:inline ml-1">Delete</span>
                          </button>
                        </div>
                      </td>
                    </tr>
                  ))
                )}
              </tbody>
            </table>
          </div>
        </div>

        {/* Pagination Container */}
        {pagination && pagination.totalPages > 1 && !searchTerm && (
          <div className="flex justify-center md:justify-end mt-6">
            <div className="join shadow-sm border border-base-200 rounded-lg bg-base-100">
              <Pagination
                currentPage={currentPage}
                totalPages={pagination.totalPages}
                totalItems={pagination.totalItems}
                itemsPerPage={pagination.itemsPerPage}
                onPageChange={setCurrentPage}
              />
            </div>
          </div>
        )}

        {/* Add/Edit Modal */}
        <div className={`modal ${isModalOpen ? "modal-open" : ""}`} style={{ zIndex: 999 }}>
          <div className="modal-box">
            <button type="button" onClick={closeModal} className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">
              ✕
            </button>
            
            <h3 className="font-bold text-xl mb-4 border-b border-base-200 pb-2">
              {editingId ? "Edit Examination Details" : "Create New Examination"}
            </h3>

            <form onSubmit={handleSubmit} className="py-2 space-y-4">
              <div className="form-control w-full">
                <label className="label">
                  <span className="label-text font-semibold text-base-content">Examination Name</span>
                </label>
                <input
                  type="text"
                  name="examinationName"
                  value={formData.examinationName}
                  onChange={handleInputChange}
                  required
                  className="input input-bordered w-full focus:input-primary"
                  placeholder="e.g., First Term Examination"
                />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="form-control w-full">
                  <label className="label">
                    <span className="label-text font-semibold text-base-content">Start Date</span>
                  </label>
                  <input
                    type="date"
                    name="startDate"
                    value={formData.startDate}
                    onChange={handleInputChange}
                    required
                    className="input input-bordered w-full focus:input-primary"
                  />
                </div>

                <div className="form-control w-full">
                  <label className="label">
                    <span className="label-text font-semibold text-base-content">End Date</span>
                  </label>
                  <input
                    type="date"
                    name="endDate"
                    value={formData.endDate}
                    onChange={handleInputChange}
                    required
                    className="input input-bordered w-full focus:input-primary"
                  />
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="form-control w-full">
                  <label className="label">
                    <span className="label-text font-semibold text-base-content">Start Time</span>
                  </label>
                  <input
                    type="time"
                    name="startTime"
                    value={formData.startTime}
                    onChange={handleInputChange}
                    required
                    className="input input-bordered w-full focus:input-primary"
                  />
                </div>

                <div className="form-control w-full">
                  <label className="label">
                    <span className="label-text font-semibold text-base-content">End Time</span>
                  </label>
                  <input
                    type="time"
                    name="endTime"
                    value={formData.endTime}
                    onChange={handleInputChange}
                    required
                    className="input input-bordered w-full focus:input-primary"
                  />
                </div>
              </div>

              <div className="modal-action mt-6">
                <button type="button" onClick={closeModal} className="btn btn-ghost">
                  Cancel
                </button>
                <button type="submit" disabled={loading} className="btn btn-primary min-w-[100px]">
                  {loading ? <span className="loading loading-spinner loading-sm"></span> : "Save Examination"}
                </button>
              </div>
            </form>
          </div>
          
          <div className="modal-backdrop" onClick={closeModal}>
            <button type="button" className="cursor-default">close</button>
          </div>
        </div>
      </div>
    </div>
  );
}