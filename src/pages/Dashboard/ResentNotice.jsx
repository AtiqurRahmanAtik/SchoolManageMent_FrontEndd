...
export default function ResentNotice() {
  const {
    recentNotices,
    pagination,
  ...
  const tableHeaders = [
    ...
    { id: "noticeName", label: "Notice Name", className: "py-4" },
    { id: "noticeTitle", label: "Notice Title", className: "py-4" },
  ...
  // Initial fetch using fetchRecentNoticesByBranch
  useEffect(() => {
    // Passing undefined as the first argument uses the default 'branch' from the hook
    fetchRecentNoticesByBranch(undefined, currentPage, limitPerPage, searchTerm);
  }, [fetchRecentNoticesByBranch, currentPage, limitPerPage, searchTerm]);
  ...
  // --- Modal Handlers ---
  const openModal = (noticeItem = null) => {
    if (noticeItem) {
      setEditingId(noticeItem._id);
      setFormData({ 
        noticeName: noticeItem.noticeName || "",
        noticeTitle: noticeItem.noticeTitle || "",
      ...
      setEditingId(null);
      setFormData({ noticeName: "", noticeTitle: "", date: "", description: "", image: "" });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await updateRecentNotice(editingId, formData);
      } else {
        await createRecentNotice(formData);
      }
      fetchRecentNoticesByBranch(undefined, currentPage, limitPerPage, searchTerm);
      closeModal();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this notice?")) {
      try {
        await removeRecentNotice(id);
        if (recentNotices.length === 1 && currentPage > 1) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      {/* Header Section */}
      <Mtitle
        title="Recent Notices Management"
        middlecontent={
          <span className="text-sm text-base-content/70 hidden md:inline-block">
            Manage and organize campus recent notices
          </span>
        }
        rightcontent={
      ...
      {/* Main Table Card - Removed overflow-x-auto to prevent scrollbar and show all data */}
              ...
              {noticeLoading ? (
                ...
                <tr>
                  <td colSpan={tableHeaders.length} className="py-12 text-center">
                    <MtableLoading data={recentNotices} />
                ...
                recentNotices?.map((noticeItem) => (
                  <tr key={noticeItem._id} className="hover">
                    ...
                    {/* Notice Name Column */}
                    <td className="py-4">
                      <div className="font-semibold text-base text-base-content">{noticeItem.noticeName}</div>
                    </td>

                    {/* Notice Title Column - Removed truncate to show full text */}
                    <td className="py-4 whitespace-normal min-w-[200px]">
                    ...
                    {/* Date Column */}
                    <td className="py-4">
                      <div className="text-sm text-base-content whitespace-nowrap">{noticeItem.date}</div>
                    </td>

                    {/* Description Column - Removed truncate to show full text */}
                    <td className="py-4 whitespace-normal min-w-[250px]">
                      <div className="text-sm text-base-content/80">
      ...
      <div className={`modal ${isModalOpen ? "modal-open" : ""}`} style={{ zIndex: 999 }}>
        <div className="modal-box max-w-2xl">
          ...
          <h3 className="font-bold text-xl mb-4 border-b border-base-200 pb-2">
            {editingId ? "Edit Notice Details" : "Create New Notice"}
          </h3>
          ...
          <form onSubmit={handleSubmit} className="py-2">
            ...
            <div className="form-control w-full mb-4">
              <label className="label">
                <span className="label-text font-semibold text-base-content">Notice Name</span>
              </label>
              <input
                type="text"
                name="noticeName"
                ...
                placeholder="Enter notice name (e.g., Recent Notice)"
              />
            </div>
            ...
            <div className="form-control w-full mb-4">
              <label className="label">
                <span className="label-text font-semibold text-base-content">Notice Title</span>
              </label>
              <input
                type="text"
                name="noticeTitle"
            ...
            <div className="form-control w-full mb-4">
              ...
              <input
                ...
                onChange={handleInputChange}
                className="input input-bordered w-full focus:input-primary"
                placeholder="https://example.com/notice-image.jpg (Optional)"
              />
              {formData.image && (
            ...
            <div className="modal-action mt-6">
              ...
              <button type="submit" disabled={noticeLoading} className="btn btn-primary min-w-[100px]">
                {noticeLoading ? <span className="loading loading-spinner loading-sm"></span> : "Save Notice"}
              </button>
            </div>
          </form>
        </div>
