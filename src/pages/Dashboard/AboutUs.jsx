...
export default function AboutUsManagement() {
  ...
  // Initial fetch using fetchAboutUsByBranch
  useEffect(() => {
    // Passing undefined as the first argument uses the default 'branch' from the hook
    fetchAboutUsByBranch(undefined, currentPage, limitPerPage, searchTerm);
  }, [fetchAboutUsByBranch, currentPage, limitPerPage, searchTerm]);
  ...
  // --- Modal Handlers ---
  const openModal = (aboutUsItem = null) => {
    if (aboutUsItem) {
      setEditingId(aboutUsItem._id);
      setFormData({ 
        headline: aboutUsItem.headline,
        image: aboutUsItem.image,
        description: aboutUsItem.description
      });
    } else {
      setEditingId(null);
      setFormData({ headline: "", image: "", description: "" });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await updateAboutUs(editingId, formData);
      } else {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this entry?")) {
      try {
        await removeAboutUs(id);
        if (aboutUsList.length === 1 && currentPage > 1) {
      ...
      <Mtitle
        ...
        middlecontent={
          <span className="text-sm text-base-content/70 hidden md:inline-block">
            Manage and organize About Us section content
          </span>
        }
        rightcontent={
              ...
              {aboutUsLoading ? (
                ...
                <tr>
                  <td colSpan={tableHeaders.length} className="py-12 text-center">
                    <MtableLoading data={aboutUsList} />
                    <div className="flex flex-col items-center justify-center text-base-content/50 mt-[-40px]">
                      ...
                      <p className="text-sm">Click "Add New Content" to create one.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                aboutUsList?.map((aboutUsItem) => (
                  <tr key={aboutUsItem._id} className="hover">
                    ...
                    <td className="py-4">
                      <div className="avatar">
                        <div className="w-16 h-16 rounded shadow border border-base-200">
                          <img 
                            src={aboutUsItem.image} 
                            alt={aboutUsItem.headline} 
                    ...
                    {/* Headline Column */}
                    <td className="py-4">
                      <div className="font-semibold text-base text-base-content">{aboutUsItem.headline}</div>
                    </td>

                    {/* Description Column */}
                    <td className="py-4">
                      <div className="text-sm text-base-content/80 max-w-sm truncate" title={aboutUsItem.description}>
                    ...
                    <td className="py-4 text-right pr-6">
                      <div className="join justify-end">
                        ...
                        <button
                          onClick={() => handleDelete(aboutUsItem._id)}
      ...
      <div className={`modal ${isModalOpen ? "modal-open" : ""}`} style={{ zIndex: 999 }}>
        <div className="modal-box">
          ...
          <h3 className="font-bold text-xl mb-4 border-b border-base-200 pb-2">
            {editingId ? "Edit Content Details" : "Create New Content"}
          </h3>
          ...
          <form onSubmit={handleSubmit} className="py-2">
            ...
            <div className="modal-action mt-6">
              ...
              <button type="submit" disabled={aboutUsLoading} className="btn btn-primary min-w-[100px]">
                {aboutUsLoading ? <span className="loading loading-spinner loading-sm"></span> : "Save Content"}
              </button>
            </div>
          </form>
        </div>
